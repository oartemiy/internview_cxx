...
std::string JwtService::GenerateToken(const boost::uuids::uuid& user_id, const std::string& role,
    ...
    auto token =
        jwt::create()
            ...
            .set_payload_claim("password_version", jwt::claim(std::to_string(password_version)))
            .sign(jwt::algorithm::hs256{secret_key_});
    return token;
}

JwtService::JWTParse JwtService::VerifyToken(const std::string& token) const {
    ...
    auto password_version_clain = decoded.get_payload_claim("password_version");
